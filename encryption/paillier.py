...
from encryption.util import invert, powmod, getprimeover, isqrt
...
if __name__ == '__main__':
    ...
    y1 = 90
    z1 = 3
    x = pubkey.encrypt_3d_matrix(x1)
    y = pubkey.encrypt_3d_matrix(y1)
    t = x + y * 0.5

    print(privkey.decrypt(t))
    t = np.array([1.0, 2.0], dtype=np.float)
    print(type(t[0]))
    x = pubkey.encrypt_3d_matrix(t[0])
    t = np.array([1, 0])
    print(type(t[0]))
    x = pubkey.encrypt_3d_matrix(int(t[0]))
