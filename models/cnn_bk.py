...
class ConvolutionLayer(Layer):
    ...
    def __init__(self, filter_size=None, n_out_channels=None, stride_size=None, padding_mode=None):
    ...
    def _set_layer_variable_initializer(self, input_feature_map_shape):
        ...
        self.filter_initializer = tf.truncated_normal(
            shape=[self.filter_size, self.filter_size, self.n_in_channels, self.n_out_channels], stddev=stddev, dtype=tf.float32)
...
class SimpleCNN(BaseModel):
    ...
    def build(self, input_shape, representation_dim, class_num, lr):
    ...
    def _build_objective_layer(self):
        ...
        with tf.control_dependencies(update_ops):
            self.back_train_op = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(
                loss=self.feature_representation,
    ...
    def backpropogate(self, X, y, in_grad):
        self.session.run(self.back_train_op,
