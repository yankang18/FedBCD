        ...
        for e in range(args.epochs):
            ...
            for i_batch in range(n_batch):
                ...
                for j in range(args.n_local):
                    ...
                    grad_a = calculate_grad_with_d(w_a_local, x_a_batch, d, args.lbda)
                for j in range(args.n_local):
                    # print("B epoch: {0}, local: {1}".format(e, j))
                    # w_a = w_a_local             ####  !!! important cyclic or asy-cyclic
                    w_b_local = w_b_local + args.lr * grad_b
                    grad_b, d = calculate_grad_with_label(w_a, x_a_batch, w_b_local, x_b_batch, y_batch, args.lbda)
                w_a = w_a_local
                w_b = w_b_local
